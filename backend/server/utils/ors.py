...
def calculate_gc_dist(lon1, lat1, lon2, lat2):
...
def detect_uphill_segments(route: list[dict[str, float]]) -> list[dict[str, Any]]:
    ...
    while i < n - 1:
        ...
        while j < n:
            dist = calculate_gc_dist(route[j-1]["lng"], route[j-1]["lat"], route[j]["lng"], route[j]["lat"])
            ...
            if (
                (total_distance >= MIN_SHORT_DISTANCE and avg_slope >= MIN_GROSS_ANGLE)
                or (total_distance >= MIN_LONG_DISTANCE and avg_slope >= MIN_KLEIN_ANGLE)
            ):
                j += 1  # keep expanding
            else:
