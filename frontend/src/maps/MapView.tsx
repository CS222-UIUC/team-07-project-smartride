...
import MapPanel from "./MapPanel.tsx";
import { useRouteOperations } from "./manage/operations.ts";
import NavigationArrow from "./widgets/NavigationArrow.tsx";
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        {/* Can only add points and routes if not readonly */}
        {!readonly && (
          <ClickHandler
            onClick={(lat, lng) => {
              void wrappedAddPoint(lat, lng);
        ...
        {/* Split */}
        {traveledPoints && traveledPoints.length > 1 && (
          <Polyline positions={traveledPoints} color="blue" weight={6} />
        )}
        {remainingPoints && remainingPoints.length > 1 && (
