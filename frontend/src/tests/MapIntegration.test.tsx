...
const useSetupRouteOperations = (initialData: { points: Array<Point>; segments: Array<RouteSegment> }) => {
  const {
    points,
    segments,
    addPoint,
  ...
  return { points, segments, addPoint, removePoint, reorderPoints, togglePointType };
};
...
vi.mock('@/maps/MapView', () => {
  const MockMapView: React.FC<{
    onRouteDataChange: (data: { points: Array<{ id: string; lat: number; lng: number; label: string; ele: number; type: string }>; segments: Array<RouteSegment> }) => void;
    ...
    <div
      ...
      onClick={() => {
        onRouteDataChange({
          points: [{ id: '1', lat: 10, lng: 20, label: 'Point 1', ele: 0, type: 'default' }],
          segments: [],
        });
      }}
    >
...
const TestWrapper = () => {
  const { points, addPoint } = useSetupRouteOperations({ points: [], segments: [] });
  return (
    <>
      <MapView
...
describe('map click behavior', () => {
  it('click sequence 1/2/3, PointMarker displays correct colors: green → blue → (green, grey, blue)', async () => {
    ...
    expect(markerElements[0]).toHaveAttribute('data-icon', expect.stringContaining('marker-icon-green'));
    ...
    expect(markerElements[0]).toHaveAttribute('data-icon', expect.stringContaining('marker-icon-green'));
    expect(markerElements[1]).toHaveAttribute('data-icon', expect.stringContaining('marker-icon-blue'));
    ...
    expect(markerElements[0]).toHaveAttribute('data-icon', expect.stringContaining('marker-icon-green'));
    expect(markerElements[1]).toHaveAttribute('data-icon', expect.stringContaining('marker-icon-grey'));
    expect(markerElements[2]).toHaveAttribute('data-icon', expect.stringContaining('marker-icon-blue'));
